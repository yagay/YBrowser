...
internal class GeckoUploadStager(context: Context) {
    ...
    fun stage(uris: List<Uri>): Array<Uri>? {
        ...
        return runCatching {
            var copiedBytes = 0L
            val staged = uris.mapIndexed { index, uri ->
                val itemDir = File(promptDir, index.toString())
                check(itemDir.mkdirs())

                val file = File(itemDir, safeFileName(uri))
                val input = resolver.openInputStream(uri)
                    ?: error("Selected file cannot be opened: $uri")
                ...
                input.use { source ->
                    file.outputStream().use { output ->
                        ...
                        while (true) {
                            ...
                            check(sessionRoot.usableSpace > MIN_FREE_BYTES) {
    ...
    private companion object {
        const val ROOT_DIR = "gecko-file-uploads"
